...
{
    public class LateComingRecomApvDataService : ILateComingRecomApvDataService
    {
        readonly IConfiguration _configuration;
        private readonly OracleDBConnection OracleDBConnection;
        private readonly DBHelper dbHelper;

        public LateComingRecomApvDataService(IConfiguration configuration)
        {
        ...
        public List<RecomApv> GetRecomApvListServiceData(string year, string toBrowse, string recomOrApv, string UserID)
        {
            ...
            if (recomOrApv == "recom")
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_SUMMARY_RCMND_DIS";
            }
            else if (recomOrApv == "apv")
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_SUMMARY_APV_DIS";
            }
            else
            {
            }
                ...
                if (recomOrApv == "recom")
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        recomapvList.Add(new RecomApv
                        {
                            ...
                            Late_recom_allow = row["LT_RCMD_ALLOW"].ToString(),
                            Late_aprv_allow = row["LT_APRV_ALLOW"].ToString()
                        });
                    }
                }
                else if (recomOrApv == "apv")
        ...
        public List<RecomApvData> GetRecomApvTableServiceData(string year, string comp_code, string toBrowse, string recomOrApv, string UserID)
        {
            ...
            if (recomOrApv == "recom")
            {
                pack = "PKG_ATDF_LRE02.F_LT_RECMD_DISPLAY";
            }
            else if (recomOrApv == "apv")
            {
                pack = "PKG_ATDF_LRE02.F_LT_APRV_DISPLAY";
            }
                ...
                if (toBrowse == "P" && recomOrApv == "recom")
                {
                    foreach (DataRow row in dt.Rows)
                    {
                        recomApvTableDataList.Add(new RecomApvData
                        {
                            ...
                            ATD_LT_CODE = row["ATD_LT_CODE"].ToString(),
                            ...
                            LT_APLN_DATE = row["LT_APLN_DATE"].ToString(),
                            LT_CAUSE_NAME = row["LT_CAUSE_NAME"].ToString(),
                            LT_DATE = row["LT_DATE"].ToString(),
                            EXPECTED_IN_TIME = row["EXPECTED_IN_TIME"].ToString(),
                            EXPECTED_LT_IN_MIN = row["EXPECTED_LT_IN_MIN"].ToString(),
                            VISIT_PLACE = row["VISIT_PLACE"].ToString(),
                            RCMD_EMP_NAME = row["RCMD_EMP_NAME"].ToString(),
                            APRV_EMP_NAME = row["APRV_EMP_NAME"].ToString(),
                            LT_RCMD_STUS = row["LT_RCMD_STUS"].ToString(),
                            LT_APLN_STATUS = row["LT_APLN_STATUS"].ToString(),
        ...
        public Response_data SubmitRecomApvServiceData(List<RecomApvData> infoList)
        {
            ...
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_RECOMMEND";
            }
            else if (infoList[0].recomOrApv == "recom_Rej")
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_REJECT";
            }
            else if (infoList[0].recomOrApv == "apv")
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_APPROVE";
            }
            else if (infoList[0].recomOrApv == "apv_Rej")
            {
                pack = "PKG_ATDF_LRE02.PRCS_LT_NOT_APPROVE";
            }
                ...
                using (OracleConnection connection = new OracleConnection(OracleDBConnection.ConnectionStringRead()))
                {
                    ...
                    using (OracleCommand command = new OracleCommand(pack, connection))
                    {
                        ...
                        try
                        {
                            ...
                            command.Parameters.Add("P_LT_APLN_EMP_CODE", OracleDbType.NVarchar2).Value = infoList[i].EMP_CODE;
                            command.Parameters.Add("P_ATD_LT_CODE", OracleDbType.NVarchar2).Value = infoList[i].ATD_LT_CODE;
                            ...
                            statusandmessage = dbHelper.GetStatusAndMessage("ATDF_LRE02", returnValue.ToString());
                            connection.Close();
                            connection.Dispose();
                        }
                        catch (Exception ex)
