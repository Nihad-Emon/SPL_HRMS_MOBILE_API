...
{
    [Route("api/OutofficeRecomApv")]
    [ApiController]
    public class OutofficeRecomApproveController : ControllerBase
    {
        private IOutofficeRecomApproveService _outOfficeRecomApproveService;

        public OutofficeRecomApproveController(IOutofficeRecomApproveService outofficeRecomApproveService)
        {
            _outOfficeRecomApproveService = outofficeRecomApproveService;
        }
        ...
        public IActionResult Post(string year, string toBrowse, string recomOrApv, string UserID)
        {
            try
            {
                ...
                _outOfficeRecomApproveService.GetRecomApvListService(year, toBrowse, recomOrApv, UserID, out recomApvList);
        ...
        public IActionResult Post(string year, string comp_code, string toBrowse, string recomOrApv, string UserID)
        {
            try
            {
                ...
                _outOfficeRecomApproveService.GetRecomApvTableService(year, comp_code, toBrowse, recomOrApv, UserID, out recomApvTableDataList);
        ...
        [HttpPost]
        [Route("submit_outoff_recom_apv")]
        public IActionResult Post([FromBody] List<RecomApvData> infoList)
        {
            try
            {
                var status = new Response_data();
                status = _outOfficeRecomApproveService.SubmitRecomApvService(infoList);
